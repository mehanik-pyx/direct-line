...
TODO: Необязательный пункт: подгрузка элементов через REST API
частями (например по 50 шт.) в виджет
1. -инпут с лимитом
2. кнопки переключения
3. задержку перед запросом
4. можно обновлять по вотчеру а можно по @change на инпуте
